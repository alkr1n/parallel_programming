...
void recording_statistics(std::chrono::duration<double> lead_time, const std::vector<std::vector<double>>& C, std::string output_file) {
	...
	if (out.is_open()) {
		...
		if (out.tellp() == 0) {
			out << "Size,Time_Seconds,Operations\n";
		}
	}
	if (out.is_open()) {
		out << n << ","
...
int main() {
	...
	recording_statistics(lead_time, C, "result_statistic.csv");
	return 0;
}